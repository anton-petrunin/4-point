...
After this resealing, the left-hand side in the required inequality can be written as $-\rho_{\bm{x}}(u)$.
Since
\[-\rho_{\bm{x}}(u)\le \rho_{\bm{x}}(v)+\rho_{\bm{x}}(w),\]
it remains to estimate $\rho_{\bm{x}}(v)+\rho_{\bm{x}}(w)$.
...
\]
where
$\tilde d_v$ and $\tilde d_w$ are distances from a vertex of the model triangle $\tilde\triangle(x_1x_2x_3)_{\EE^2}$ to a point on the opposite side that divides it in a certain ratio (that depends on $v$ and $w$, respectively).
Note that for a general 4-point array, $\tilde d_v$ and $\tilde d_w$ do not exceed the largest side $\tilde\triangle(x_1x_2x_3)_{\EE^2}$.
