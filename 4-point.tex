...
Note that due to Walds Theorem (see \cite[Exercise 10.7]{AKP-2024}) we can assume that the $4$-point array
comes from a model $k$-plane\footnote{i.e, a complete simply connected 2-dimensional Riemannian manifold of constant curvature $k$.} with $k \ge -1$. This in particular implies that it is either (a) is isometric to a $4$-point subset of some sphere or (b) is bi-Lipshitz equivalent with the constant $1 + C \delta^2$ to a $4$-point subset of a Euclidean plane, where $C > 0$ is some absolute constant. Denote by $K_\delta\subset W_4$ the minimal closed convex cone that includes all associated forms of these metrics.
...
any element of $K_\delta$ is bi-Lipshitz equivalent with the constant $1 + C \delta^2$ to some element of $K_0$;
