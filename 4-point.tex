...
%\makeatletter\let\orig@section\section\renewcommand{\section}[1]{\orig@section{#1.}}\usepackage{environ}\RenewEnviron{figure}[1][]{}\RenewEnviron{figure*}[1][]{}\RenewEnviron{wrapfigure}[1][]{}\pagestyle{empty}\makeatletter\let\ps@plain\ps@empty\makeatother\def\emph{\textit}\usepackage[none]{hyphenat}
...
Let $\bm{x}\z=(x_1,\dots,x_n)$ be an $n$-point array in a metric space $X$ and let $a_{i,j}$ be the components of a symmetric $n{\times}n$ matrix.
...
with $a_{i,j}=-\lambda_i\cdot\lambda_j$ for a real array $(\lambda_1,\dots, \lambda_n)$ such that $\lambda_1+\dots+\lambda_n=0$, which are discussed in Section~\ref{par:rank-one}.

In Section~\ref{Four-point arrays}, we prove a version of a theorem by Abraham Wald \cite[§ 7]{wald} describing the metrics of all possible 4-point arrays in Alexandrov spaces with nonnegative and nonpositive curvature.
...
Further, in Section~\ref{Alexandrov's comparison}, we prove that a single nondegenerate inequality of negative type defines Alexandrov spaces with either nonnegative or nonpositive curvature.
...
The proof is a straightforward combination of an argument by Takashi Sato \cite{sato} and a variation due to the first two authors \cite{lebedeva-petrunin-2010};
...
Let $V_n\z=\RR^{n-1}$ be the Euclidean space with a choice of unit-edge regular simplex $\triangle$, whose vertices are $y_1,\dots,y_n$.
...
We are allowed to not distinguish between $\rho_{\bm{x}}$ and the corresponding semimetric on $\bm{x}=(x_1,\dots,x_n)$.
...
The first reason comes from the triangle inequality, which is equivalent to $\rho_{\bm{x}}(w)\ge 0$ for any vector $w$ lying in a 2-face of $\triangle$.
...
An inequality of type $\rho_{\bm{x}}(v)\ge 0$ for a fixed vector $v\in V_n$ will be called a \emph{rank-one} inequality.
...
We know that $L\le 0 $ holds for any 4-point array in $X$ and need to show the same for $L'\le 0$.
...
\eqlbl{eq:L'=L+L}
\]
In particular, $L \le 0$ implies $L' \le 0$ as required.
...
To prove \ref{eq:CBB-CBA}$'$, suppose $x_4$ lies on $[x_1x_2]$ and divides it in the ratio $(1-\alpha_n):\alpha_n$, then
...
\end{align*}
If we assume $\alpha=\alpha_n$ and $\gamma=\beta_n$ in \ref{eq:lambda-inq}, then instead of zero in the right-hand side, we get $\delta_n^2\cdot E_n$:
\[
\begin{aligned}
...
\\
+\alpha_n\cdot(1-\beta_n)\cdot\beta_n\cdot|x_1-x_3|^2-(1-\alpha_n)\cdot(1-\beta_n)\cdot |x_2-x_4|^2&\le \delta_n^2\cdot E_n
\end{aligned}
...
\\
&\le |x_3-x_4|^2 +  \tfrac{\delta_n^2}{\beta_n}\cdot E_n+\beta_n\cdot S.
\end{aligned}
\]
...
\eqlbl{eq:L<En-hats}
\]
Note that the new array ${\bm{\lambda}}'_n$ is covered in cases~\ref{in} or~\ref{side}.
...
\eqlbl{eq:uvw}
\]
we have that $u$ points in a direction from $N$.
Therefore, $\rho_{\bm{x}}(u)\ge0$ is an inequality of negative type $(3,1)$.

Let us rescale $u$, $v$, and $w$ so that the inequality $\rho_{\bm{x}}(u)\ge 0$ (which as we know has negative type $(3,1)$)
...
Now we have
\[\sum_{i,j=1}^4\lambda_i\cdot\lambda_j\cdot|x_i-x_j|_X^2=-\rho_{\bm{x}}(u)\le \rho_{\bm{x}}(v)+\rho_{\bm{x}}(w),\]
and it remains to estimate $\rho_{\bm{x}}(v)+\rho_{\bm{x}}(w)$.

Since the inequalities $\rho_{\bm{x}}(v)\ge 0$ and $\rho_{\bm{x}}(w)\ge 0$ have type $(2,1)$, we can choose $\eps_v\ge 0$ and $\eps_w\ge 0$ such that
...
Note that $\tilde d_v$ and $\tilde d_w$ do not exceed the largest side $\tilde\triangle(x_1x_2x_3)_{\EE^2}$; in particular
\begin{align*}
\tilde d_v^2&\le |x_1-x_2|_X^2+|x_2-x_3|_X^2+|x_3-x_1|_X^2,
\\
\tilde d_w^2&\le |x_1-x_2|_X^2+|x_2-x_3|_X^2+|x_3-x_1|_X^2.
\end{align*}
Therefore, it is sufficient to find an appropriate upper bound on $\eps_v+\eps_w$.
...
\[\eps_v\cdot \tilde d_v^2+\eps_w\cdot \tilde d_w^2=\theta(v)+\theta(w)
<
-\theta(u)
\le
...
To do this, we fix $n$, and thus fix an inequality which corresponds to it, say $I_n = N_n - E_n \le 0$, where $N_n$ is an inequality of negative type and $E_n$ is the error term.
We fix $k = k(n)$ such that $\theta^2\le \beta_n^{2^k}<\theta$, say for $\theta=\tfrac{99}{100}$.
...
Since $\beta_n\to 1^-$, we can choose $k=k(n)$ such that $k\to \infty$ as $n\to \infty$ and $\theta^2\le \beta_n^{2^k}<\theta$, say for $\theta=\tfrac{99}{100}$ and
\[
