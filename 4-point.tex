...
Fix $\theta=\tfrac{99}{100}$ and choose the largest $k = k(n)$ such that $ \beta_n^{k}\ge \theta$.
Note that $k(n)$ is defined for all sufficiently large~$n$;
morevoer $\beta_n^k\to \theta$ as $n\to\infty$.
As in the proof of Claim~\ref{clm:1=>2}, let $L_n'$ be the left-hand side in~\ref{eq:lambda} for the array
\[\hat{\bm{\lambda}}_n=(\alpha_n\cdot(1-\beta_n^{k}),(1-\alpha_n)\cdot(1-\beta_n^{k}),\beta_n^{k},-1),\]
and define
\[E_n'=10\cdot\alpha_n\cdot(1-\alpha_n)\cdot\beta_n^{k}\cdot(1-\beta_n^{k})^2 \cdot (|x_1-x_2|_X^2+|x_2-x_3|_X^2+|x_3-x_1|_X^2).\]

Note that a partial limit of $\hat{\bm{\lambda}}_n$ cannot get into the corner of the triangle.
Therefore, it suffices to prove the following inequlaitly
\[
...
Following calculations in Claim~\ref{clm:1=>2},
we find that
$L_n'(x_1,x_2,x_3,x_{3+k})$ can be written as a telescoping sum
of $L_n(x_1,x_2,z_{2+i},z_{3+i})$ with appropriate positive coefficients;
here $x_3,\dots,x_{3+k}$ are points on the geodesic $[x_3,x_{3+k}]$.
Applying \ref{eq:L'=L+L}, we see that the coefficient in front of each $L_n(x_1,x_2,x_{2+i},x_{3+i})$ does not exceed $k$.
Therefore,
...
\]
for all~$i$, and
\[L_n'(x_1,x_2,x_3,x_4)\le 100\cdot 4^k\cdot \delta_n^2\cdot E_n(x_1,x_2,x_3).
\]
Furthermore, since $1>\beta_n\ge\beta_n^{k}\ge \theta$,
we have $1-\beta_n^{k}\ge \tfrac k2\cdot(1-\beta_n)$, and hence
\[
4^{k}\cdot E_n(x_1,x_2,x_3)\le 10\cdot E_n'(x_1,x_2,x_3);
\]
that is, \ref{eq:L<En-hats} holds for $x_1,x_2,x_3,x_{3+k}$ --- a contradiction.
\qeds
