...
As we will see, $n=4$ is the first interesting case.
In particular, nonnegative and nonpositive curvature in the sense of Alexandrov can be described by inequalities of that type.
Namely they are described by the so-called inequalities of negative type $(3,1)$ and $(2,2)$ respectively.
Therefore, it worth to investigate the whole class of these inequalies.
We will collect observations about these inequalities and their effect on the length spaces, including globalization theorems.
...
it is uniquely defined and remembers all $|x_i-x_j|^2_X$
...
Let $K\subset W$ be a convex cone.
...
Suppose $K$ is a closed convex cone in $W$ and $K=K'$.
If $K$ is nontrival, then $\mathcal{M}_K$ contains all Euclidean spaces.
\end{thm}

\parit{Proof.}
We can assume that $K\ne \{0\}$, otherwise there is nothing to prove.
In particular, $\mathcal{M}_K$ contains a space, say $X$ with distinct points.
Consider its ultracompletion $X^\omega\supset X$;
since $K$ is closed, $X^\omega\in \mathcal{M}_K$.

The space $X^\omega$ is geodesic.
...
\begin{thm}{Corollary}
Suppose $K$ is a closed convex cone in $W$ and $K=K'$.
Then either $K\supset Q$ or $K$ is the trivial cone $K=\{0\}$.
\end{thm}
...
\paragraph{Rank-one inequalities.}
...
These inequalities are also known as inequalities of \emph{negative type}.
...
The equators divide the hemisphere into triangles and quadrangles.
...
A vector on an equator corresponds to a inequality of negative type $(2,1)$, which follows from the triangle inequality.
...
If $X$ satisfies all inequalities of negative type $(2, 2)$, then it admits an isometric embedding into the product $T\times\RR^3$, where $T$ denotes the tripod;
...
More general five-point versions of this corollary were already proved by Tetsu Toyoda \cite{toyoda} and the first two authors \cite{lebedeva-petrunin-2024}.
...
Consider a minimal form $\tilde\rho\le \rho$ such that all $(2,1)$ inequalities hold for $\tilde\rho$.
In particular, there is a (semi)metric on $X$ with the associated form $\tilde\rho$; denote by
$\tilde X$ the corresponding metric space.
If $X$ satisfies all inequalities of negative type $(3, 1)$ or $(2,2)$, then so does $\tilde X$.

Observe that $X$ embeds into $\tilde X\times \RR^3$.
Hence it is sufficient to show that $\tilde X$ embeds into $T$ or $r\cdot \mathbb{S}^1$ for some $r>0$.

Let $N\subset \mathbb{S}^2$ be a set where $\tilde\rho$ is negative and let $\bar N$ be its closure.
From above, $N$ is nonempty.

Since all $(2,1)$-inequalities hold for $\tilde\rho$,
it must be nonnegative in on 4 equators $e_1,e_2,e_3,e_4$ in the directions of faces of $\triangle$.
Since $\tilde\rho$ is minimal, $\bar N$ has to touch $e_i$ at least at 3 directions (up to sign). 
Otherwise there is a linear function, say $\ell$, that vanish at all common points of $\bar N$ and $e_i$ for all $i$.
...
In the case of triangle $\bar N$ touches all of its sides.
...
In any case cases the array is embeddable in $r\cdot \mathbb{S}^1$, where $r=|x_1-x_4|/\pi$.
\qeds

Following the convention from \cite{lebedeva-petrunin-2010},
the equalities at the end of the proof can be encoded by the first three diagrams;
\begin{figure}[h!]
\centering
\vskip-0mm
...
\vskip-0mm
\end{figure}
if three indexes $x_1$, $x_2$, and $x_3$ uppear in this order on a smooth line, then it means that \[|x_1-x_2|+|x_2-x_3|=|x_1-x_3|.\]
The last diagram says that the array is isometric to an array on the real line.
From the proof one should see that this classifies all extremal rays of the convex cone of all forms associated to 4-point arrays in metric spaces.
...
\begin{enumerate}[(i)]
\item If the inequality is of negative type $(2,2)$, then $\mathcal{M}_K$ consists all length spaces with nonnegative curvature in the sense of Alexandrov.
...
These notes consider particular cases of $(2,2)$ and $(3,1)$ inequalities, namely for $\lambda$-arrays $(1,1,-1,-1)$ and $(1,1,1,-3)$, respectively.
...
for some $\alpha,\beta$ distinct from $0$ and $1$.
...
We can assume that the array is enumerated as on the pictures, 
so for the inequalities of type $(2,2)$ segment $[x_1,x_3]$ intersects $[x_2,x_4]$ and 
for inequalities of type $(3,1)$ point $x_4$ lies inside of triangle $x_1x_2x_3$.
\begin{figure}[ht!]
\vskip-0mm
\centering
...
Moreover, the inequality defines the array uniquely up to an affine transformation. 




Note that we get equality in \ref{eq:lambda} for this array.
Moreover, this defines bijection between $\bm{x}$-arrays up to affine traansformation and $\lambda$-arrays up to multiplication by a nonzero coefficient;
...
Indeed, passing tho the ultracompletion, we may assume that $X$ is geodesic.
Choose 4 points $x_1,x_2,x_3,x_5\in X$ and let $x_4$ be a point on a geodesic $[x_3,x_5]$ that divides it in the same ration as in the 5-point configuration in the plane.
...
The claim implies that intequality for $\lambda$-array $(\lambda_1,\lambda_2,\lambda_3,-1)$ implies the inequality for the array $(\lambda_1\cdot (1+\lambda_3),\lambda_2\cdot (1+\lambda_3),\lambda_3^2,-1)$.
Iterating the procedure, we get all the inequalities with $\lambda$-arrays
$(\lambda_1\cdot (1+\lambda_3^n),\lambda_2\cdot (1+\lambda_3),\lambda_3^n,-1)$.
\qeds
...
We say that a metric space $X$ is said to meet local $K$-comparison if any point $x\in X$ admits a neighborhood $U$ such that $K$-comparison for any $n$-point array in $U$.

Suppose local $K$-comparison implies $K$-comparison for any length space.
Then we say that \emph{globalization holds} for the $K$-comparison.
...
Therefore, local $K$-comparison holds for any circle $r\cdot \mathbb{S}^1$, and therefore for the prduct space $r\cdot \mathbb{S}^1\times\RR^3$ for any $r>0$.
It remains to apply the proposition in~\ref{Four-point arrays}.
\qeds

\parit{Proofo of the theorem.}
