\begin{abstract}
We investigate how quadratic constraints on the six distances among the points of every quadruple affect the geometry of length spaces.
\end{abstract}
...
\[(\alpha\cdot (1-\beta),(1-\alpha)\cdot(1\z-\beta), \beta,-1)\] holds for any point array in $X$.
...
This implies that the array either (a) is isometric to a $4$-point subset of some sphere or (b) is bi-Lipschitz equivalent with the constant $1 + C\cdot \delta^2$ to a $4$-point subset of a Euclidean plane, where $C > 0$ is some absolute constant.
...
any element of $K_\delta$ is bi-Lipschitz equivalent with the constant $1 + C \cdot\delta^2$ to some element of $K_0$;
...
\eqlbl{eq:L<En-hats}
\]
Note that the new array ${\bm{\lambda}}'_n$ is covered in case~\ref{in} or~\ref{side}.
