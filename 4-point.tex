...
By \ref{cor:Four-point arrays}, $K_0$ describes alл metrics on 4-point арраыс in Alexandrov spaces with nonnegative curvature.
...
\[\angk{x_1}{x_2}{x_3}+\angk{x_1}{x_2}{x_4}+\angk{x_1}{x_3}{x_4}\le 2\cdot\pi+\delta\]
remains valid after any relabeling of $\{x_1,x_2,x_3,x_4\}$.
Denote by $K_\delta$ the minimal convex cone that includes all associated forms of these metrics.
...
From now on, we can assume that for any $\delta>0$ there is a from $\rho_\delta\in K_\delta\setminus K$.
Thus we can choose quadratic intequality that does not hold for $\rho_\delta$, but holds for any form in $K$.
Our inequality can be written as $\langle \omega,\xi \rangle\ge 0$, where $\xi\in W$ is an associated form, $\omega$ is a fixed form in $W$.
...
Let $N\subset \SSS^2\subset V$ be the set of unit vectors $v$ such that $\rho_\delta(v)<0$.
...
By the spectral thereom, $\omega$ and $u^2$ can be diagonalized in common basis.
It follows that, the inequality $\langle \omega,\xi \rangle\ge 0$ can be rewritten as
...
It follows that $v_1\in N$ and $\xi(v_1)\ge0$ defines an inequality of negative type $(3,1)$.
...
let $(\alpha,(1-\alpha), 0,-1)$ be its $\lambda$-array.
...
Consider new metric on $\{x_1,x_2,x_3,x_4\}$ redefining the distance $|x_2-x_3|:=(1-\eps)\cdot|x_2-x_3|$ and leaving the remaining five distances the same; denote by $\xi'$ its asociated form.
If $\delta$ is sufficiently small, then $\xi'(v_1)\ge0$.
Now arguing as in the proof of \ref{prop:Alexandrov's comparison}, we get \ref{eq:CBB-CBA}.
