...
\vskip-0mm
\end{figure}
if three labels, say $x_1$, $x_2$, and $x_3$, uppear in this order on a smooth line, then it means that \[|x_1-x_2|+|x_2-x_3|=|x_1-x_3|.\]
...
The claim implies that intequality for $\lambda$-array $(\alpha\cdot (1-\beta),(1-\alpha)\cdot(1-\beta), \beta,-1)$ implies the inequality for the $\lambda$-array $(\alpha\cdot (1-\beta^2), (1-\alpha)\cdot(1-\beta^2), \beta^2,-1)$.
...
in particular $\delta>0$ can be taken arbitrarily small.
\[
\begin{aligned}
...
Let us subtract \ref{eq:trig-inq} from \ref{eq:lambda-inq} and divide the result by $\delta$:
\[
\begin{aligned}
...
So far we get it only for one value of $\alpha$, but iterating

Since $x_1$, $x_2$, and $x_4$ lie on one geodesic so that \ref{eq:trig-inq} holds,
depending on sing of $1-\alpha$ it is the defining inequality of nonnegative or nonpositive curvature in the sense of Alexandrov.
More presisely, assume this inequality holds for some $\alpha\in (0,1)$, then appying iteration we can choose $\alpha$ from the follwing sequence of nimbers $(1-\alpha)$, $\alpha^2$,  $\alpha\cdot (1-\alpha)$, $(1-\alpha)^2$, and so on.
This sequence is dense in $(0,1)$.
Therefore, the \ref{eq:CBB-CBA} holds for any $\alpha\in (0,1)$; the latter is equivalent to the point-on-side comparison for nonnegative curvature; see \cite[8.14]{AKP-2024}.
